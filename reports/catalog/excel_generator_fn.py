# reports/catalog/excel_generator_fn.py
...
def generate_catalog_excel_fn(
    ...
    def outline_row_span(row, c_start, c_end):
        for cidx in range(c_start, c_end + 1):
            ws.cell(row=row, column=cidx).border = Border(
                ...
                left=medium_side if cidx == c_start else Side(border_style='thin'),
                right=medium_side if cidx == c_end else Side(border_style='thin')
            )
    ...
    def outline_merged_span_edges(row_top, row_bottom, col_left, col_right):
        for cidx in range(col_left, col_right + 1):
            ...
            ws.cell(row=row_top, column=cidx).border = Border(
                top=medium_side,
                left=medium_side if cidx == col_left else existing.left,
                right=medium_side if cidx == col_right else existing.right,
        ...
        for r in range(row_top, row_bottom + 1):
            ...
            ws.cell(row=r, column=col_left).border = Border(
                left=medium_side, top=existing.top, bottom=existing.bottom, right=existing.right
            )
            existing = ws.cell(row=r, column=col_right).border
            ws.cell(row=r, column=col_right).border = Border(
                right=medium_side, top=existing.top, bottom=existing.bottom, left=existing.left
            )

    def outline_cell(r, cidx):
        ws.cell(row=r, column=cidx).border = Border(
            top=medium_side, bottom=medium_side, left=medium_side, right=medium_side
        )
    ...
    ws["AR1"].border = Border(top=Side(style="medium"), bottom=Side(style="medium"),
    ...
    def outline_col_span(col_idx, r_start, r_end):
        for r in range(r_start, r_end + 1):
            ...
            top_side = existing.top if getattr(existing.top, 'style', None) == 'medium' else (medium_side if r == r_start else Side(border_style='thin'))
            bottom_side = existing.bottom if getattr(existing.bottom, 'style', None) == 'medium' else (medium_side if r == r_end else Side(border_style='thin'))
            left_side = existing.left if getattr(existing.left, 'style', None) == 'medium' else medium_side
            right_side = existing.right if getattr(existing.right, 'style', None) == 'medium' else medium_side
    ...
    if last_girl_row:
        for col_idx in range(2, 11):
            ...
            cell.border = Border(
                ...
                top=cell.border.top, bottom=medium_side
            )
